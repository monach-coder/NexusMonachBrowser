...
public static class LocalPortScanner
{
    ...
    internal static LocalPortEntry Classify(LocalPortInfo info)
    {
        ...
        var (risk, note, severity) = (info.Protocol, info.Port) switch
        {
            ...
            (_, 445) => ("SMB", "Общий доступ к файлам — атакуемый порт", 2),
            (_, 3389) => ("RDP", "Удалённый рабочий стол — высокий риск взлома", 2),
            (_, 5900) => ("VNC", "Удалённое управление — высокий риск", 2),
            (_, 22 or 23) => ("Telnet/SSH", "Удалённый терминал открыт", 2),
